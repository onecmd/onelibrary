...
/**
 * This is the model class for table "{{club_finance}}".
 *
 * The followings are the available columns in table '{{club_finance}}':
 * @property string $id
 ...
 */
class ClubFinance extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			...
			array('club_id, record_id', 'length', 'max'=>10),
			array('title', 'length', 'max'=>100),
			array('detail', 'length', 'max'=>1000),
			array('record_time, pay_time', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, club_id, title, detail, is_income, value, record_time, pay_time, record_id', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return ClubFinance the static model class
	 */
